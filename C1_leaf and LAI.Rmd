...
StemP%>%
   ggplot(aes(x=Shootbiomass1, y=StemWt,label=GrowthRotation,colour=Name))+geom_text()+theme_bw()+xlab("Shootbiomass")+ylab("StemWt")+
  geom_smooth(method = "lm", se = TRUE,linetype=1 , colour="black")+
 #facet_grid(GrowthSeason~Rotation)+
  ggtitle("Iversen_8Waterirr")+
  theme(axis.title.x=element_text(face="bold",colour="black",size = 12))+
...
lines(StemP1$Shootbiomass2, fitted(m), lty = 1, col = "red", lwd = 0.5)
```
```{r}
