...
Resp.lm<-lm(ChangeWt~IRW, data = Root.dfper)
summary(Resp.lm)
new.root <-data.frame(IRW=2570)
predict(Resp.lm, newdata = new.root, interval='confidence') 
...
Rootmet <- read.table(paste0(obsData, "RootMet.txt"),
  ...
  #dplyr::filter(ExpName!="Iversen_8WaterirrGs_5Rt_3")
...
```
###shoot RUE for increasing and decreasing potoperiod
```{r}
UniExp <- unique(Tem.factor$ExpName)

fPAR.df.Inc <- data.frame()
...
for(i in 1:length(UniExp))
{
 ...
 mutate(PARi=radn*fPAR)%>% 
    unique()%>%
     ##accmulated function
 mutate(Production.day0 = PARi*5.0*T.factor)%>%               ###RUE is 0.50g/MJ
 mutate(RUE.0.5 = cumsum(Production.day0))%>%
 mutate(Production.day1 = PARi*5.5*T.factor)%>%               ###RUE is 0.55g/MJ
 mutate(RUE.0.55 = cumsum(Production.day1))%>%
 mutate(Production.day2 = PARi*6.0*T.factor)%>%               ###RUE is 0.6g/MJ
 mutate(RUE.0.6 = cumsum(Production.day2))%>%
 mutate(Production.day3 = PARi*6.5*T.factor)%>%               ###RUE is 0.65g/MJ
 mutate(RUE.0.65 = cumsum(Production.day3))%>%
 mutate(Production.day4 = PARi*7*T.factor)%>%               ###RUE is 0.7 g/MJ
 mutate(RUE.0.7 = cumsum(Production.day4))%>% 
 mutate(Production.day5 = PARi*7.5*T.factor)%>%               ###RUE is 0.75g/MJ
 mutate(RUE.0.75 = cumsum(Production.day5))%>%  
 mutate(Production.day6 = PARi*8*T.factor)%>%               ###RUE is 0.8 g/MJ
 mutate(RUE.0.8 = cumsum(Production.day6))%>%    
 mutate(Production.day7 = PARi*8.5*T.factor)%>%               ###RUE is 0.85g/MJ
 mutate(RUE.0.85 = cumsum(Production.day7))%>% 
 mutate(Production.day8 = PARi*9*T.factor)%>%               ###RUE is 0.9g/MJ
 mutate(RUE.0.9 = cumsum(Production.day8))%>%
 mutate(Production.day9 = PARi*9.5*T.factor)%>%               ###RUE is 0.95g/MJ
 mutate(RUE.0.95 = cumsum(Production.day9))%>% 
 mutate(Production.day10 = PARi*10*T.factor)%>%               ###RUE is 1 g/MJ
 mutate(RUE.1.0 = cumsum(Production.day10))%>%  
 mutate(Production.day11 = PARi*10.5*T.factor)%>%               ###RUE is 1.05g/MJ
 mutate(RUE.1.05 = cumsum(Production.day11))%>%  
 mutate(Production.day12 = PARi*11*T.factor)%>%               ###RUE is 1.1 g/MJ
 mutate(RUE.1.1 = cumsum(Production.day12))%>%  
 mutate(Production.day13 = PARi*11.5*T.factor)%>%               ###RUE is1.15 g/MJ
 mutate(RUE.1.15 = cumsum(Production.day13))%>%
 mutate(Production.day14 = PARi*12*T.factor)%>%               ###RUE is 1.2 g/MJ
 mutate(RUE.1.2 = cumsum(Production.day14))%>% 
 mutate(Production.day15 = PARi*12.5*T.factor)%>%               ###RUE is 1.25g/MJ
 mutate(RUE.1.25 = cumsum(Production.day15))%>%  
 mutate(Production.day16 = PARi*13*T.factor)%>%               ###RUE is 1.3g/MJ
 mutate(RUE.1.3 = cumsum(Production.day16))%>%  
 mutate(Production.day17 = PARi*13.5*T.factor)%>%               ###RUE is 1.35g/MJ
 mutate(RUE.1.35 = cumsum(Production.day17))%>% 
 mutate(Production.day18 = PARi*14.0*T.factor)%>%               ###RUE is 1.4g/MJ
 mutate(RUE.1.4 = cumsum(Production.day18))%>% 
 mutate(Production.day19 = PARi*14.5*T.factor)%>%               ###RUE is 1.45g/MJ
 mutate(RUE.1.45 = cumsum(Production.day19))%>% 
 mutate(Production.day20 = PARi*15*T.factor)%>%                ###RUE is 1.5g/MJ
 mutate(RUE.1.5 = cumsum(Production.day20))%>%
 mutate(Production.dayA = PARi*5)%>%                         ###RUE is 0.5 g/MJ
 mutate(Production.sumA = cumsum(Production.dayA))%>%  
 mutate(Production.dayB = PARi*5.5)%>%                       ###RUE is 0.55 g/MJ
 mutate(Production.sumB = cumsum(Production.dayB))%>% 
 mutate(Production.dayC = PARi*6)%>%                          ###RUE is 0.6 g/MJ
 mutate(Production.sumC = cumsum(Production.dayC))%>% 
 mutate(Production.dayD = PARi*6.5)%>%                        ###RUE is 0.65 g/MJ
 mutate(Production.sumD = cumsum(Production.dayD))%>% 
 mutate(Production.dayE  = PARi*7)%>%                          ###RUE is 0.7 g/MJ
 mutate(Production.sumE  = cumsum(Production.dayE))%>%  
 mutate(Production.dayF = PARi*7.5)%>%                        ###RUE is 0.75 g/MJ
 mutate(Production.sumF = cumsum(Production.dayF))%>%  
 mutate(Production.dayG = PARi*8)%>%                          ###RUE is 0.8 g/MJ
 mutate(Production.sumG = cumsum(Production.dayG))%>% 
 mutate(Production.dayH = PARi*8.5)%>%                         ###RUE is 0.85 g/MJ
 mutate(Production.sumH = cumsum(Production.dayH))%>% 
 mutate(Production.dayI = PARi*9 )%>%                            ###RUE is 0.9 g/MJ
 mutate(Production.sumI = cumsum(Production.dayI))%>% 
 mutate(Production.dayJ = PARi*9.5)%>%                           ###RUE is 0.95g/MJ
 mutate(Production.sumJ = cumsum(Production.dayJ))%>%
 mutate(Production.dayK = PARi*10)%>%                            ###RUE is 1 g/MJ
 mutate(Production.sumK = cumsum(Production.dayK))%>%
 mutate(Production.dayL = PARi*10.5)%>%                         ###RUE is 1.05g/MJ
 mutate(Production.sumL = cumsum(Production.dayL))%>%
 mutate(Production.dayM = PARi*11)%>%                            ###RUE is 1.1g/MJ
 mutate(Production.sumM = cumsum(Production.dayM))%>%
 mutate(AccPARi=cumsum(PARi))

 fPAR.df.Inc<- rbind( fPAR.df.Inc, fPAR.data.Inc)

}

summary(fPAR.df.Inc)
fPAR.df.Inc
write.csv(fPAR.df.Inc,"D:/R/fPAR.df.Inc.csv", row.names = FALSE)


```

####Increasing Pp
###RUE testing for all treatment in LL

```{r,width=8, height=8}

ShootInc.LL <- fPAR.df.Inc %>%
  ...
  filter(fPAR!=0)%>%
  dplyr::select(ExperimentID,TreatmentID,ID,Clock.Today,ExpName,shootbiomass, Tearth,Trend,RUE.0.7,RUE.0.75,RUE.0.8,RUE.0.85,RUE.0.9,RUE.0.95,RUE.1.0,RUE.1.05,RUE.1.1,RUE.1.15,RUE.1.2,RUE.1.25) %>%
  tidyr::gather("Variable","Predicted",RUE.0.7:RUE.1.25)


ShootInc.LL $Variable<-factor(ShootInc.LL$Variable, levels=c("RUE.0.7","RUE.0.75","RUE.0.8","RUE.0.85","RUE.0.9","RUE.0.95","RUE.1.0","RUE.1.05","RUE.1.1","RUE.1.15","RUE.1.2","RUE.1.25"))
...
ShootInc.LL%>%
  ...
  theme_bw()+mytheme3+
  theme(legend.title=element_blank())+
   scale_x_continuous(breaks = seq(2000, 6000, by =4000), limits=c(0,8000))+
  scale_y_continuous(breaks = seq(2000, 6000, by =2000), limits=c(0,8000))+
  ylab("Predicted")+xlab("Observed") 
  #theme(legend.title=element_blank(),legend.position = "blank")

 ggsave("D:/R/Pictures/C5/Yield/RUEshootInc.LL.png", width=8, height=8, dpi=500)
 
```
###Stistic analysis for increasing Pp
```{r}
...
ShootDec.LL <- fPAR.df.Inc %>%
  ...
  filter(Stage!="Seedling")%>%
  filter(fPAR!=0)%>%
  dplyr::select(ExperimentID,TreatmentID,ID,Clock.Today,ExpName,shootbiomass, Tearth,Trend,Production.sum0,Production.sum1,Production.sum2,Production.sum3,Production.sum4,Production.sum5,Production.sum6,Production.sum7,Production.sum8,Production.sum9,Production.sum10,Production.sum11,Production.sum12,Production.sum13,Production.sum14,Production.sum15,Production.sum16,Production.sum17,Production.sum18,Production.sum19,Production.sum20,Production.sumA,Production.sumB,Production.sumC,Production.sumD,Production.sumE,Production.sumF,Production.sumG,Production.sumH,Production.sumI,Production.sumJ,Production.sumK,Production.sumL,Production.sumM) %>%
  tidyr::gather("Variable","Predicted",Production.sum0:Production.sumM)


ShootDec.LL $Variable<-factor(ShootDec.LL$Variable, levels=c("Production.sum0","Production.sum1","Production.sum2","Production.sum3","Production.sum4","Production.sum5","Production.sum6","Production.sum7","Production.sum8","Production.sum9","Production.sum10","Production.sum11","Production.sum12","Production.sum13","Production.sum14","Production.sum15","Production.sum16","Production.sum17","Production.sum18","Production.sum19","Production.sum20","Production.sumA","Production.sumB","Production.sumC","Production.sumD","Production.sumE","Production.sumF","Production.sumG","Production.sumH","Production.sumI","Production.sumJ","Production.sumK","Production.sumL","Production.sumM"))
...
ShootDec.LL%>%
 ...
 ggsave("D:/R/Pictures/C5/Yield/RUEshootDec.LL.png", width=8, height=8, dpi=500)
 
```

###Stistic analysis for Decreasing Pp
```{r}
...
####Increasing Pp
###RUE testing for all treatment in DF

```{r,width=8, height=8}

ShootInc.DF <- fPAR.df.Inc %>%
  filter(Defoliation!="LL")%>%
  filter(Defoliation!="HH")%>%
  filter(shootbiomass!="NA")%>%
  filter(shootbiomass!=0)%>%
  filter(Trend=="Inc")%>%
  filter(Stage!="Seedling")%>%
...
ShootInc.DF $Variable<-factor(ShootInc.DF$Variable, levels=c("Production.sum0","Production.sum1","Production.sum2","Production.sum3","Production.sum4","Production.sum5","Production.sum6","Production.sum7","Production.sum8","Production.sum9","Production.sum10","Production.sum11","Production.sum12","Production.sum13","Production.sum14","Production.sum15","Production.sum16","Production.sum17","Production.sum18","Production.sum19","Production.sum20","Production.sumA","Production.sumB","Production.sumC","Production.sumD","Production.sumE","Production.sumF","Production.sumG","Production.sumH","Production.sumI","Production.sumJ","Production.sumK","Production.sumL","Production.sumM"))
...
ShootInc.DF%>%
 ...
 ggsave("D:/R/Pictures/C5/Yield/RUEshootInc.DF.png", width=8, height=8, dpi=500)
...
ShootDec.DF%>%
 ...
 ggsave("D:/R/Pictures/C5/Yield/RUEshootDec.DF.png", width=8, height=8, dpi=500)
...
```
####Increasing Pp
###RUE testing for all treatment in HH

```{r,width=8, height=8}

ShootInc.HH <- fPAR.df.Inc %>%
  filter(Defoliation=="HH")%>%
  filter(shootbiomass!="NA")%>%
  filter(shootbiomass!=0)%>%
  filter(Trend=="Inc")%>%
...
ShootInc.HH $Variable<-factor(ShootInc.HH$Variable, levels=c("Production.sum0","Production.sum1","Production.sum2","Production.sum3","Production.sum4","Production.sum5","Production.sum6","Production.sum7","Production.sum8","Production.sum9","Production.sum10","Production.sum11","Production.sum12","Production.sum13","Production.sum14","Production.sum15","Production.sum16","Production.sum17","Production.sum18","Production.sum19","Production.sum20","Production.sumA","Production.sumB","Production.sumC","Production.sumD","Production.sumE","Production.sumF","Production.sumG","Production.sumH","Production.sumI","Production.sumJ","Production.sumK","Production.sumL","Production.sumM"))
...
ShootInc.HH%>%
 ...
 ggsave("D:/R/Pictures/C5/Yield/RUEshootInc.HH.png", width=8, height=8, dpi=500)
 
```

###Stistic analysis for Decreasing Pp OF HH
```{r}
...
ShootDec.HH $Variable<-factor(ShootDec.HH$Variable, levels=c("Production.sum0","Production.sum1","Production.sum2","Production.sum3","Production.sum4","Production.sum5","Production.sum6","Production.sum7","Production.sum8","Production.sum9","Production.sum10","Production.sum11","Production.sum12","Production.sum13","Production.sum14","Production.sum15","Production.sum16","Production.sum17","Production.sum18","Production.sum19","Production.sum20","Production.sumA","Production.sumB","Production.sumC","Production.sumD","Production.sumE","Production.sumF","Production.sumG","Production.sumH","Production.sumI","Production.sumJ","Production.sumK","Production.sumL","Production.sumM"))
...
ShootDec.HH%>%
 ...
 ggsave("D:/R/Pictures/C5/Yield/RUEshootDec.HH.png", width=8, height=8, dpi=500)
...
```{r}
RUEshoot<-Biomass%>%
  dplyr::filter(shootbiomass!="NA")%>%
  mutate(Shootbiomass1=shootbiomass*0.1)

RUE.shoot<-RUEshoot%>%
  group_by(GrowthSeason,Rotation,ID,ExperimentID,Tmean,Ppm, FD, Defoliation,Trend) %>%
  do(mod = lm(Shootbiomass1~AccPARi,data=.)) %>%
  mutate(RUE.shoot = summary(mod)$coeff[2]) %>%
  dplyr::select(-mod)
...
RUE.shoot%>%
  ...
  filter(RUE.shoot>0)%>%
  #filter(Defoliation=="LL")%>%
  ggplot(aes(x=Tmean,y=RUE.shoot, color=Defoliation))+geom_point(size=2)+theme_bw()+
  facet_grid(Defoliation~Trend)+
 # geom_point(data=ObsR4, aes(x=Clock.Today1, y=Observed),colour="green",size=3)+
  #theme(legend.title=element_blank(),legend.position = "blank")+
  xlab("Mean temperature")+ylab("Shoot RUE (g/MJ)")+
 #remove grid lines 
   theme(
     panel.spacing=unit(.01, "lines"),
     panel.border = element_rect(colour = "black",size=1), 
     ...
     legend.title = element_text(colour="black", size=14, face="bold"),
     axis.text = element_text(face = "bold", vjust = 0.5, size = 14))
ggsave("D:/R/Pictures/c6/Yield/RUE.png", width=8, height=6, dpi=500)
```
