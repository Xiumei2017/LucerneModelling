---
title: "R Notebook"
output: html_notebook
---

```{r}
library(dplyr)
library(ggplot2)
library(lubridate)   
library(hydroGOF)
library(xtable)
library(knitr)
library(tidyr)
library(RSQLite)
library(agricolae)
library(scales)
library(zoo)

```
# ### load obs data
###filter out FD5 and seedling crop 
```{r}
upDir <- "D:/R/CombinedData/"
obsData <- "D:/R/CombinedData/"

obsFD <- read.table(paste0(obsData, "FallDtreatment.txt"),
                   header = TRUE)
obsFDN<- obsFD %>%
  dplyr::filter(Variable=="NodeNumber")%>%
  #dplyr::filter(FD!="FD5")%>%
  dplyr::filter(Water!="dry")%>%
  mutate(StartDate=dmy(StartDate),MidDate=dmy(MidDate),FinishDate=dmy(FinishDate),Clock.Today=dmy(Clock.Today))%>%
  mutate(GrowthSeason1=as.factor(paste0("Gs_",GrowthSeason))) %>% # creating more intuitive labels here
  mutate(Rotation1=as.factor(paste0("Rt_",Rotation))) %>%
  mutate(ExpUnitCode=as.factor(paste0(Name,GrowthSeason1,Rotation1))) %>%
  mutate(Clock.Today1 = as.POSIXct(paste(Clock.Today,Time),format="%Y-%m-%d %H:%M:%S"))%>%
  mutate(GrowthSeason2=as.factor(paste0("Y",GrowthSeason,"(",Period,")"))) %>%
 mutate(Rotation2=as.factor(paste0("R",Rotation)))
obsFDN
```
###Seedling crop
###filter out the seedling crop
```{r}
upDir <- "D:/R/CombinedData/"
obsData <- "D:/R/CombinedData/"

obsSeed <- read.table(paste0(obsData, "FallDtreatment.txt"),
                   header = TRUE)
obsSeedN<- obsSeed %>%
  dplyr::filter(Variable=="NodeNumber")%>%
  #dplyr::filter(FD=="FD5")%>%
  dplyr::filter(Water!="dry")%>%
  dplyr::filter(ExperimentID=="E4")%>%
  mutate(StartDate=dmy(StartDate),MidDate=dmy(MidDate),FinishDate=dmy(FinishDate),Clock.Today=dmy(Clock.Today))%>%
  mutate(GrowthSeason1=as.factor(paste0("Gs_",GrowthSeason))) %>% # creating more intuitive labels here
  mutate(Rotation1=as.factor(paste0("Rt_",Rotation))) %>%
  mutate(ExpUnitCode=as.factor(paste0(Name,GrowthSeason1,Rotation1))) %>%
  mutate(Clock.Today1 = as.POSIXct(paste(Clock.Today,Time),format="%Y-%m-%d %H:%M:%S"))%>%
  mutate(GrowthSeason2=as.factor(paste0("Y",GrowthSeason,"(",Period,")"))) %>%
 mutate(Rotation2=as.factor(paste0("R",Rotation)))
obsSeedN
```
###load Tt data
```{r}
upDir <- "D:/R/"
obsData <- "D:/R/TtAll/"

Tt<- read.table(paste0(obsData, "df.all.txt"),
               header = TRUE)
TtA <- Tt %>% mutate(Clock.Today=dmy(Clock.Today), ExpUnitCode=as.factor(ExpName))%>%
  dplyr::filter(Tbb==1)


obsFDNT <-merge(obsFDN,TtA,by=c("Clock.Today","ExpUnitCode")) %>%
  mutate(GrowthRotation=as.factor(paste0(GrowthSeason.x,Rotation.x)))%>%
  dplyr::filter(Variable=="NodeNumber")
  summary(obsFDNT)
  obsFDNT
  
```
####my theme
```{r}
mytheme1<-theme(
     panel.spacing=unit(.01, "lines"),
     panel.border = element_rect(colour = "black",size=1), 
     panel.grid.major = element_blank(),
     panel.grid.minor = element_blank(),
     axis.line = element_line(colour = "black",size = 1),
     axis.text.x = element_text(face = "plain", color = "black", size = 14,angle=90, vjust=0.5, hjust = 1),
     axis.text.y = element_text(face = "plain", color = "black", size = 14),
     axis.title.x=element_text(face="bold",colour="black",size = 14),
     axis.title.y=element_text(face="bold",colour="black",size = 14),
     strip.background = element_rect(colour = "black", fill = "white",size=1),
     strip.text.x = element_text(size=14, angle=0, face = "plain"), 
     strip.text.y = element_text(size=14, face="plain"),
     legend.title = element_text(colour="black", size=14, face="bold"),
     axis.text = element_text(face = "bold", vjust = 0.5, size = 14))

mytheme3<-theme(
     legend.title = element_text(colour="black", size=14, face="bold"),
     legend.text = element_text(colour="black", size = 14,face="plain"),
     panel.spacing=unit(.01, "lines"),
     panel.border = element_rect(colour = "black",size=1), 
     panel.grid.major = element_blank(),
     panel.grid.minor = element_blank(),
     axis.line = element_line(colour = "black",size = 1),
     axis.text.x = element_text(face = "plain", color = "black", size = 14, vjust=0.5, hjust = 1),
     axis.text.y = element_text(face = "plain", color = "black", size = 14),
     axis.title.x=element_text(face="bold",colour="black",size = 14),
     axis.title.y=element_text(face="bold",colour="black",size = 14),
     strip.background = element_rect(colour = "black", fill = "white",size=1),
     strip.text.x = element_text(size=14, angle=0, face = "plain"), 
     strip.text.y = element_text(size=14, face="plain"),
     axis.text = element_text(face = "bold", vjust = 0.5, size = 14),
    legend.position = "right", legend.box = "vertical")
```
###LL
##FD2
```{r,fig.height=6, fig.width=8}
  obsFDF2LLN<-obsFDNT%>%
  dplyr::filter(ID=="E5ILLF2")
  obsFDF2LLN$Rotation2<- factor(obsFDF2LLN$Rotation2,
                          levels=c("R1", "R2", "R3", "R4","R5","R6","R7","R8","R9","R10"))
   obsFDF2LLN%>%  
  ggplot(aes(x=Tt_broken_sum, y=Observed))+geom_point(size=2)+theme_bw()+xlab("Thermal time (°Cd) ")+ylab("Node number")+ggtitle(paste0("E5ILLF2","(Iversen_121DefoliationLLF2)"))  +
  scale_x_continuous(breaks = seq(0, 550, by =200), limits=c(0,450))+
  scale_y_continuous(breaks = seq(0, 18, by =6), limits=c(0,16))+
 geom_smooth(method = "lm", se = TRUE, linetype = 1, colour="blue")+
 facet_grid(GrowthSeason2~Rotation2)+mytheme1
#ggsave("D:/R/Pictures/C6/NodeNumber/Iversen_121DefoliatioLLF2node.png", width=8, height=6, dpi=500)

```
##FD10
```{r,fig.height=6, fig.width=8}
  obsFDF10LLN<-obsFDNT%>%
  dplyr::filter(ID=="E5ILLF10")
  obsFDF2LLN$Rotation2<- factor(obsFDF2LLN$Rotation2,
                          levels=c("R1", "R2", "R3", "R4","R5","R6","R7","R8","R9","R10"))
   obsFDF10LLN%>%  
  ggplot(aes(x=Tt_broken_sum, y=Observed))+geom_point(size=2)+theme_bw()+xlab("Thermal time (°Cd) ")+ylab("Node number")+ggtitle(paste0("E5ILLF10"))  +
  scale_x_continuous(breaks = seq(0, 550, by =200), limits=c(0,450))+
  scale_y_continuous(breaks = seq(0, 18, by =6), limits=c(0,16))+
 geom_smooth(method = "lm", se = TRUE, linetype = 1, colour="blue")+
 facet_grid(GrowthSeason2~Rotation2)+mytheme1
ggsave("D:/R/Pictures/C6/NodeNumber/Iversen_121DefoliatioLLF10node.png", width=8, height=6, dpi=500)

```

###SS
##FD2
```{r,fig.height=6, fig.width=8}
 obsFDF2SSN<-obsFDNT%>%
  dplyr::filter(ID=="E5ISSF2")
 obsFDF2SSN$Rotation2<- factor(obsFDF2SSN$Rotation2, levels=c("R1", "R2", "R3", "R4", "R5","R6", "R7","R8","R9","R10"))
   obsFDF2SSN%>%  
  ggplot(aes(x=Tt_broken_sum, y=Observed))+geom_point(size=2)+theme_bw()+xlab("Thermal time (°Cd) ")+ylab("Node number")+ggtitle(paste0("E5ISSF2"))  +
  scale_x_continuous(breaks = seq(0, 550, by =200), limits=c(0,450))+
  scale_y_continuous(breaks = seq(0, 18, by =6), limits=c(0,16))+
 geom_smooth(method = "lm", se = TRUE, linetype = 1, colour="blue")+
 facet_grid(GrowthSeason2~Rotation2)+mytheme1
ggsave("D:/R/Pictures/C6/NodeNumber/Iversen_121DefoliationSSFD2node.png", width=8, height=6, dpi=500)

```
##FD10
```{r,fig.height=6, fig.width=8}
 obsFDF10SSN<-obsFDNT%>%
  dplyr::filter(ID=="E5ISSF10")
 obsFDF10SSN$Rotation2<- factor(obsFDF10SSN$Rotation2, levels=c("R1", "R2", "R3", "R4", "R5","R6", "R7","R8","R9","R10"))
   obsFDF10SSN%>%  
  ggplot(aes(x=Tt_broken_sum, y=Observed))+geom_point(size=2)+theme_bw()+xlab("Thermal time (°Cd) ")+ylab("Node number")+ggtitle(paste0("E5ISSF10"))  +
  scale_x_continuous(breaks = seq(0, 550, by =200), limits=c(0,450))+
  scale_y_continuous(breaks = seq(0, 18, by =6), limits=c(0,16))+
 geom_smooth(method = "lm", se = TRUE, linetype = 1, colour="blue")+
 facet_grid(GrowthSeason2~Rotation2)+mytheme1
ggsave("D:/R/Pictures/C6/NodeNumber/Iversen_121DefoliationSSFD10node.png", width=8, height=6, dpi=500)

```

###HH
##FD2
```{r,fig.height=6, fig.width=8}
  obsFD2HHN<-obsFDNT%>%
  dplyr::filter(ID=="E5IHHF2")
  obsFD2HHN$Rotation2<- factor(obsFD2HHN$Rotation2, levels=c("R1", "R2", "R3", "R4", "R5","R6", "R7","R8","R9","R10"))
  obsFD2HHN%>%  
  ggplot(aes(x=Tt_broken_sum, y=Observed))+geom_point(size=2)+theme_bw()+xlab("Thermal time (°Cd) ")+ylab("Node number")+ggtitle(paste0("E5IHHF2"))  +
  scale_x_continuous(breaks = seq(0, 500, by =200), limits=c(0,500))+
  scale_y_continuous(breaks = seq(0, 18, by =6), limits=c(0,16))+
 geom_smooth(method = "lm", se = TRUE, linetype = 1, colour="blue")+
 facet_grid(GrowthSeason2~Rotation2)+mytheme3
ggsave("D:/R/Pictures/C6/NodeNumber/Iversen_121DefoliationHHFD2node.png", width=8, height=6, dpi=500)

```

##FD10
```{r,fig.height=6, fig.width=8}
  obsFD10HHN<-obsFDNT%>%
  dplyr::filter(ID=="E5IHHF10")
  obsFD10HHN$Rotation2<- factor(obsFD10HHN$Rotation2, levels=c("R1", "R2", "R3", "R4", "R5","R6", "R7","R8","R9","R10"))
  obsFD10HHN%>%  
  ggplot(aes(x=Tt_broken_sum, y=Observed))+geom_point(size=2)+theme_bw()+xlab("Thermal time (°Cd) ")+ylab("Node number")+ggtitle(paste0("E5IHHF10"))  +
  scale_x_continuous(breaks = seq(0, 500, by =200), limits=c(0,500))+
  scale_y_continuous(breaks = seq(0, 18, by =6), limits=c(0,16))+
 geom_smooth(method = "lm", se = TRUE, linetype = 1, colour="blue")+
 facet_grid(GrowthSeason2~Rotation2)+mytheme3
ggsave("D:/R/Pictures/C6/NodeNumber/Iversen_121DefoliationHHFD10node.png", width=8, height=6, dpi=500)

```
###phyllochron

```{r}
 phyllochronFD <- obsFDNT%>%
  dplyr::filter(Defoliation.x!="HH")%>%
  mutate(GrowthSeason=as.factor(GrowthSeason.x),Rotation=as.factor(Rotation.x))%>%
  mutate(GrowthRotation=as.factor(paste0(GrowthSeason,Rotation)))%>%
  group_by(Name,GrowthSeason,Rotation,Tmean,Ppm,Tbb,ID,GrowthRotation) %>%
    do(mod = lm(Tt_broken_sum~Observed,data=.))%>%
  mutate(PhyollchronFD= summary(mod)$coeff[2])%>%
  dplyr::select(-mod)
  
```


#load Rotation and Growth season
```{r,fig.height=4, fig.width=8}
library(plyr)
phyll <- "D:\\R\\"
StartGrazing <- read.table(paste0(phyll, "ExperimentList.txt"), 
                      header = TRUE)
StartGrazing1<-StartGrazing %>%
  mutate(GrowthRotation= as.factor(paste0(GrowthSeason,Rotation)))
PhyPpFD<- merge(StartGrazing1,phyllochronFD, by=c("Name","GrowthRotation"))
 
lm_eqn2 <- function( PhyPpFD){
  m <- lm(PhyollchronFD~Ppm ,PhyPpFD);
  eq <- substitute(italic(y) == Ppm +PhyollchronFD %.% italic(Ppm)*","~~italic(R)^2~"="~r2, 
                   list(Ppm = format(coef(m)[1], digits = 2), 
                        PhyollchronFD= format(coef(m)[2], digits = 2), 
                        r2 = format(summary(m)$r.squared, digits = 3)))
  as.character(as.expression(eq));                 
}


eqs <- ddply(PhyPpFD,.(Trend,FD),lm_eqn2)
#df2 <- data.frame(eq = unclass(eqns), Collection = as.numeric(names(eqns)))
#ylab(bquote(bold('LEAR ('*m^2~m^-2~'°Cd'^-1*')')))

PhyPpFD%>%
  #filter(FD=="FD10")%>%
  ggplot(aes(x=Ppm, y=PhyollchronFD,colour=factor(ID),label=GrowthRotation))+geom_text()+
  theme_bw()+
  theme(legend.title=element_blank(),legend.position = "blank")+xlab("Mean photoperiod (h)")+ylab(bquote(bold('Phyllochron ('*'°Cd'~'main node'^-1*')')))+
  geom_text(data = eqs, aes(x =13.5, y = 81, label = V1), 
          color = 'black',  parse = TRUE, size=5)+
  geom_smooth(method = "lm", se = TRUE, linetype = 1, colour="darkgrey")+
  facet_grid(FD~Trend)+
   scale_y_continuous(breaks = seq(0, 70, by =20), limits=c(0,81))+
  mytheme3+theme(legend.title = element_blank())
   detach(package:plyr)

ggsave("D:/R/Pictures/C6/NodeNumber/phylloLLSS.png", width=8, height=6, dpi=500)
```

```{r}
 phyllochronFD.all <- obsFDNT%>%
  mutate(GrowthSeason=as.factor(GrowthSeason.x),Rotation=as.factor(Rotation.x))%>%
  mutate(GrowthRotation=as.factor(paste0(GrowthSeason,Rotation)))%>%
  group_by(Name,GrowthSeason,Rotation,Tmean,Ppm,Tbb,ID,GrowthRotation) %>%
    do(mod = lm(Tt_broken_sum~Observed,data=.))%>%
  mutate(PhyollchronFD= summary(mod)$coeff[2])%>%
  dplyr::select(-mod)
  
```

###all
```{r,fig.height=4, fig.width=6}
phyllochronFD.all%>%
  mutate(GrowthRotation=as.factor(paste0(GrowthSeason,Rotation)))%>%
  ggplot(aes(x=Ppm, y=PhyollchronFD,colour=factor(ID),label=GrowthRotation))+geom_text()+
  #geom_point(size=2)+
  theme_bw()+xlab("Photoperiod")+ylab("Phyllochron")+
 geom_smooth(method = "lm", se = TRUE, linetype = 1, colour="blue")+
 mytheme3+
   theme(legend.title=element_blank(),legend.position = "blank")
 
```
####Statistic measurement for linear regression
```{r}
obsFDNT
unicode <- unique(obsFDNT$ExpUnitCode)

PhyFD.all <- data.frame()
 
for(i in 1:length(unicode)){
  
  obsFDNT1 <- obsFDNT%>%
  filter(ExpUnitCode==unicode[i])
  
    obsFDNT1.sub <- obsFDNT1 %>%
  group_by(Name,GrowthSeason.x,Rotation.x,Collection,Tmean,Ppm,ID,ExperimentID) %>%
    do(mod.broken = lm(Tt_broken_sum~Observed,data=.))%>%
    #do(mod.fick = lm(Tt_fick_sum~Observed,data=.))%>%
  mutate(R2Br = summary(mod.broken)$r.squared)%>%
  mutate(PhylloBr = summary(mod.broken)$coeff[2])%>%
  mutate(P=anova(mod.broken)$'Pr(>F)'[1])%>% 
  mutate(intcp= summary(mod.broken)$coeff[1])%>%  
  dplyr::select(-mod.broken)

  PhyFD.all <- rbind(PhyFD.all, obsFDNT1.sub)
}
  
PhyFD.all 
write.csv(PhyFD.all ,"D:/R/PhyllochronData/PhybrokenFD.all.csv", row.names = FALSE)


```
###Join with the flowering data

```{r}
upDir <- "D:/R/"
obsData <- "D:/R/"

flowering <- read.table(paste0(obsData, "Flowering.txt"),
                   header = TRUE)
flower <- flowering%>%
  mutate(StartDate=dmy(StartDate),BVDate=dmy(BVDate),FloDate=dmy(FloDate))
  

mergeHHFD<-merge(obsFDNT,flower,by=c("Name","ID","StartDate"))%>%
   mutate(Dev=ifelse(Clock.Today<=BVDate,"Veg","Repro"))
   

```
###Nodenumber and Phyllochron
```{r,fig.height=6, fig.width=10}
# mergeHHFD_Veg<-mergeHHFD%>%
#   dplyr::filter(Dev=="Veg")%>%
#   dplyr::filter(Variable=="NodeNumber")
# mergeHHFD_Veg%>%  
#   ggplot(aes(x=Tt_broken_sum, y=Observed), colour=factor(Name))+geom_point(size=2)+theme_bw()+xlab("Thermal time(°Cd)")+ylab("Node number")+ggtitle(paste0("E5IHHF5","(Iversen_121DefoliationHHFD5)"))+
#  geom_smooth(method = "lm", se = TRUE, linetype = 1, colour="blue")+
#  facet_grid(GrowthSeason.x~Rotation.x)+
#   scale_x_continuous(breaks = seq(0, 1000, by =400), limits=c(0,1000))+
#   scale_y_continuous(breaks = seq(0, 20, by =6), limits=c(0,25))+
#  geom_smooth(method = "lm", se = TRUE, linetype = 1, colour="blue")+
#  facet_grid(GrowthSeason2~Rotation2)+mytheme1
# #ggsave("D:/R/Pictures/C5/Node/Iversen_121DefoliationHHFD5node.png", width=8, height=6, dpi=500)
#   
```

```{r}
#  mergeHHFD_V <- mergeHHFD_Veg %>%
#   mutate(GrowthSeason=as.factor(GrowthSeason.x),Rotation=as.factor(Rotation.x))%>%
#   group_by(Name,GrowthSeason,Rotation,Tmean,Ppm,Tbb) %>%
#     do(mod = lm(Tt_broken_sum~Observed,data=.))%>%
#   mutate(R2Br = summary(mod)$r.squared)%>%
#   mutate(Veg = summary(mod)$coeff[2])%>%
#   #mutate(PhylloBr = summary(mod.broken)$coeff[2])%>%
#   mutate(P=anova(mod)$'Pr(>F)'[1])%>% 
#   mutate(intcp= summary(mod)$coeff[1])%>%  
#   dplyr::select(-mod)
# write.csv(mergeHHFD_V ,"D:/R/PhyllochronData/mergeHHFD_V.all.csv", row.names = FALSE)
```


```{r,fig.height=6, fig.width=10}
# mergeHHFD_Repro<-mergeHHFD%>%
#   dplyr::filter(Dev=="Repro")%>%
#   dplyr::filter(Variable=="NodeNumber")
# mergeHHFD_Repro%>%
#   ggplot(aes(x=Tt_broken_sum, y=Observed), colour=factor(Name))+geom_point(size=2)+theme_bw()+xlab("Thermal time(°Cd)")+ylab("Node number")+ggtitle(paste0("E5IHHF5","(Iversen_121DefoliationHHFD5)"))+
#  geom_smooth(method = "lm", se = TRUE, linetype = 1, colour="blue")+
#  facet_grid(GrowthSeason2~Rotation2)+
#   scale_x_continuous(breaks = seq(0, 1000, by =400), limits=c(0,900))+
#   scale_y_continuous(breaks = seq(10, 26, by =6), limits=c(10,25))+
#  mytheme3+
#    theme(legend.title=element_blank(),legend.position = "blank")
#ggsave("D:/R/Pictures/C5/Node/Iversen_121DefoliationHHFD5node12.png", width=8, height=6, dpi=500)
```

```{r}
#  mergeHHFD_R <- mergeHHFD_Repro %>%
#   mutate(GrowthSeason=as.factor(GrowthSeason.x),Rotation=as.factor(Rotation.x))%>%
#   #dplyr::filter(GrowthSeason!="2"|Rotation!="1") %>%
#   group_by(Name,GrowthSeason,Rotation,Tmean,Ppm,Tbb) %>%
#     do(mod = lm(Tt_broken_sum~Observed,data=.))%>%
#    mutate(R2Br = summary(mod)$r.squared)%>%
#   mutate(Reg= summary(mod)$coeff[2])%>%
#   #mutate(PhylloBr = summary(mod.broken)$coeff[2])%>%
#   mutate(P=anova(mod)$'Pr(>F)'[1])%>% 
#   mutate(intcp= summary(mod)$coeff[1])%>% 
#   dplyr::select(-mod)
# write.csv(mergeHHFD_R ,"D:/R/PhyllochronData/mergeHH_R.all.csv", row.names = FALSE)
#   
# mergePhyFD<-merge(mergeHHFD_V, mergeHHFD_R,by=c("Name","GrowthSeason","Rotation","Tmean","Ppm","Tbb"))%>%
#   dplyr::select(Name,GrowthSeason,Rotation,Tmean, Ppm,Tbb, Veg,Reg) %>%
#   tidyr::gather("Variable","PhylloFD",Veg:Reg)
#   
```

```{r,fig.height=4, fig.width=6}
# mergePhyFD$Variable<-factor(mergePhyFD$Variable, levels = c("Veg","Reg"))
# 
# 
# mergePhyFD%>%
#   mutate(GrowthRotation=as.factor(paste0(GrowthSeason,Rotation)))%>%
#   filter(GrowthRotation!="21")%>%
#   filter(GrowthRotation!="33")%>%
#   ggplot(aes(x=Ppm, y=Phyllo,colour=factor(Name),label=GrowthRotation))+geom_text()+
#   #geom_point(size=2)+
#   theme_bw()+xlab("Photoperiod (h)")+ylab(bquote(bold('Phyllochron ('*'°Cd'~'main node'^-1*')')))+ggtitle("E5IHHF5(Iverson_121DefoliationHHFDFD5)")+
#  geom_smooth(method = "lm", se = TRUE, linetype = 1, colour="blue")+
#  facet_wrap(~Variable,ncol = 2)+mytheme3+
#    theme(legend.title=element_blank(),legend.position = "blank")

#ggsave("D:/R/Pictures/C5/Node/Iversen_121DefoliationHHFD5phyllochron.png", width=8, height=4, dpi=500)
```
## Define stats function

* Using Gauch et al. 2003 (Model evaluation by comparison of model-based predictions and measured values. Agron. J. 95, 1442-1446) 
```{r Stats, include = TRUE, echo=FALSE, warning=FALSE, fig.height=8, fig.width=8}

# # R2
# testDF <- data.frame(a=c(1,2,3,4,5), b=c(10,20,10,40,50))
# 
# myR2 <- function(p,o) {
#  return(summary(lm(p~o, na.action=na.exclude))$r.squared) 
# }
# 
# testDF %>%
#   summarise(thisR2 = myR2(a,b))

# gauch MSE components
gauchStats <- function(sim, meas) {

  n_s <- length(sim)
  n_m <- length(meas)
  model <- lm(meas~sim)
  sim_sq <- sum((sim - mean(sim))^2)
  mes_sq <- sum((meas - mean(meas))^2)
  r2 <- summary(model)$r.squared
  slope <- model$coefficients[[2]]

  sb <- (sum(mean(meas)) - sum(mean(sim)))^2
  nu <- (1-slope)^2 * (sim_sq/n_s)
  lc <- (1-r2) * (mes_sq/n_m)
  msd <- sb+nu+lc

  sb_r <- round((sb/msd)*100,1)
  nu_r <- round((nu/msd)*100,1)
  lc_r <- round((lc/msd)*100,1)

  msd_r <- sb_r+nu_r+lc_r

  # select which variables to output
  out <- c(sb_r,nu_r,lc_r, msd_r, round(r2*100,1))

  return(out)

}
```

## Test stats functions used

```{r}
s <- c(4231.972,3935.604,3779.652,3627.687,3363.499,3230.566,2868.114,2868.827)
m <- c(4987.66,5636.09,4754.06,4114.53,4141.72,3704.06,5142.19,4762.03)

x <- gauchStats(s,m)

tempDf <- data.frame(statName=c("SB","NU","LC","r_MSD","R2"), statValue=x)
# kable(tempDf, digits= 2)
tempDf2 <- data.frame(Predicted=s, Observed=m)

x <- tempDf2 %>%
  summarise(
    n = n(),
    r2 = gauchStats(Predicted,Observed)[5],
  #  rmse = round(rmse(Predicted,Observed),0),
    r_rmse = round(rmse(Predicted,Observed)/mean(Observed)*100,1),
    nse = round(NSE(Predicted,Observed),1),
    sb = gauchStats(Predicted,Observed)[1],
  nu = gauchStats(Predicted,Observed)[2],
  lc = gauchStats(Predicted,Observed)[3]
  ) %>% 
  t() 

df <- data.frame(stat = row.names(x),statvalue = x[,1])

df %>%
  kable(format = "markdown")
```

## Load simulated database
## create function to read data (Justin's script)
```{r LoadSim, include = FALSE, echo=FALSE, warning=FALSE, fig.height=8, fig.width=8}
GetApsimNGTable <- function(dbLoc, table) 
{
  connection <- dbConnect(SQLite(), dbname = dbLoc, flags = SQLITE_RW)
  table <- dbReadTable(connection, table, row.names=NULL)
  dbDisconnect(connection)
  return(table)
}

```
# load address of db
# set table to be enquierd
# load table into an object
# make it a dataframe
# change date to corerct format 
# explore the df
```{r}
db.address <- "D:\\APSIMX2\\Prototypes\\Lucerne\\LucerneValidation.db"
tableName<-"Report"
DbTable <- GetApsimNGTable(db.address,tableName)
df <- as.data.frame(DbTable)
df$Clock.Today <- ymd_hms(df$Clock.Today)
str(df)
summary(df)
head(df) # simulation results
```
# get sim names (different table)
# merge names 
# remove unecessary variables
```{r}
simNameDf <- as.data.frame (GetApsimNGTable(db.address,"_Simulations"))
myDb <- merge(df, simNameDf, by.x= c("SimulationID"), by.y= c("ID"))


#str(myDb)
head(myDb)
summary(myDb)

# myDb %>%
#   dplyr::select(Name) %>%
#   unique()

```

```{r}
simD <- myDb %>%
  dplyr::select(Name,Clock.Today,LAI,Height,shootbiomass,RootWt, StemWt, LeafWt,NodeNumber) %>%
  tidyr::gather("Variable","Predicted",LAI:NodeNumber) %>%
  mutate(Name = as.factor(Name)) %>%
  mutate(Variable = as.factor(Variable)) %>%
  mutate(Clock.Today = ymd_hms(Clock.Today))

head(simD)
summary(simD)



mergedf<-merge(obsFDNT,simD,by=c("Clock.Today","Name","Variable"))
summary(mergedf)
str(mergedf)
mergedf

```
## Node number
#Time series
## obs Vs Pre for each experiment
## LL
## FD2
```{r,fig.height=4, fig.width=8}

obsFDNTLLF2<-obsFDNT%>%
  dplyr::filter(Name=="Iversen_121DefoliationLLFDFD2")

simD1a<-simD%>%
  mutate(Clock.Today = ymd_hms(Clock.Today))%>%
  dplyr::filter(Variable=="NodeNumber")%>%
  dplyr::filter(Name=="Iversen_121DefoliationLLFDFD2")%>%
  dplyr::filter(Clock.Today<"2015-01-06")

simD1b<-simD%>%
  mutate(Clock.Today = ymd_hms(Clock.Today))%>%
  dplyr::filter(Variable=="NodeNumber")%>%
  dplyr::filter(Name=="Iversen_121DefoliationLLFDFD2")%>%
  dplyr::filter(Clock.Today>"2015-01-30")
simD1<-rbind(simD1a,simD1b)
  
  
str(simD1)
simD1%>%
  #dplyr::filter(Clock.Today>"2015-01-31")%>%
ggplot(aes(x=Clock.Today,y=Predicted))+geom_line(size=1)+theme_bw()+
  facet_wrap(~ID,ncol = 1)+
  geom_point(data=obsFDNTLLF2, aes(x=Clock.Today1, y=Observed),colour="green",size=3)+
  theme(legend.title=element_blank(),legend.position = "blank")+xlab("Date")+ylab("Node number")+
  #remove grid lines 
   theme(
     panel.spacing=unit(.01, "lines"),
     panel.border = element_rect(colour = "black",size=1), 
     panel.grid.major = element_blank(),
     panel.grid.minor = element_blank(),
     axis.line = element_line(colour = "black",size = 1),
     axis.text.x = element_text(face = "plain", color = "black", size = 14, vjust=0.5, hjust = 1),
     axis.text.y = element_text(face = "plain", color = "black", size = 14),
     axis.title.x=element_text(face="bold",colour="black",size = 14),
     axis.title.y=element_text(face="bold",colour="black",size = 14),
     strip.background = element_rect(colour = "black", fill = "white",size=1),
     strip.text.x = element_text(size=14, angle=0, face = "plain"), 
     strip.text.y = element_text(size=14, face="plain"),
     legend.title = element_text(colour="black", size=14, face="bold"),
     axis.text = element_text(face = "bold", vjust = 0.5, size = 14))+
   annotate("text", x=ymd_hms("2016-07-24 12:00:00"), y=20, size = 5, label ="paste(R_RMSD == 27,'%')", parse=T)
 #ggsave("D:/R/Pictures/C6/NodeNumber/Iversen_121DefoliationLLFDFD2Node.png", width=8, height=4, dpi=500)
  

```
## FD10
```{r,fig.height=4, fig.width=8}

obsFDNTLLF10<-obsFDNT%>%
  dplyr::filter(Name=="Iversen_121DefoliationLLFDFD10")

simD2a<-simD%>%
  mutate(Clock.Today = ymd_hms(Clock.Today))%>%
  dplyr::filter(Variable=="NodeNumber")%>%
  dplyr::filter(Name=="Iversen_121DefoliationLLFDFD10")%>%
  dplyr::filter(Clock.Today<"2015-01-06")

simD2b<-simD%>%
  mutate(Clock.Today = ymd_hms(Clock.Today))%>%
  dplyr::filter(Variable=="NodeNumber")%>%
  dplyr::filter(Name=="Iversen_121DefoliationLLFDFD10")%>%
  dplyr::filter(Clock.Today>"2015-01-30")
simD2<-rbind(simD2a,simD2b)
  
  
str(simD2)
simD2%>%
  #dplyr::filter(Clock.Today>"2015-01-31")%>%
ggplot(aes(x=Clock.Today,y=Predicted))+geom_line(size=1)+theme_bw()+
  facet_wrap(~ID,ncol = 1)+
  geom_point(data=obsFDNTLLF10, aes(x=Clock.Today1, y=Observed),colour="green",size=3)+
  theme(legend.title=element_blank(),legend.position = "blank")+xlab("Date")+ylab("Node number")+
  #remove grid lines 
   theme(
     panel.spacing=unit(.01, "lines"),
     panel.border = element_rect(colour = "black",size=1), 
     panel.grid.major = element_blank(),
     panel.grid.minor = element_blank(),
     axis.line = element_line(colour = "black",size = 1),
     axis.text.x = element_text(face = "plain", color = "black", size = 14, vjust=0.5, hjust = 1),
     axis.text.y = element_text(face = "plain", color = "black", size = 14),
     axis.title.x=element_text(face="bold",colour="black",size = 14),
     axis.title.y=element_text(face="bold",colour="black",size = 14),
     strip.background = element_rect(colour = "black", fill = "white",size=1),
     strip.text.x = element_text(size=14, angle=0, face = "plain"), 
     strip.text.y = element_text(size=14, face="plain"),
     legend.title = element_text(colour="black", size=14, face="bold"),
     axis.text = element_text(face = "bold", vjust = 0.5, size = 14))+
   annotate("text", x=ymd_hms("2016-07-24 12:00:00"), y=20, size = 5, label ="paste(R_RMSD == 27.3,'%')", parse=T)
#ggsave("D:/R/Pictures/C6/NodeNumber/Iversen_121DefoliationLLFDFD10Node.png", width=8, height=4, dpi=500)
  

```
## SS
##F2
```{r,fig.height=4, fig.width=8}

obsFDNTSSF2<-obsFDNT%>%
  dplyr::filter(Variable=="NodeNumber")%>%
  dplyr::filter(Name=="Iversen_121DefoliationSSFDFD2")
  
   
 simD3a<-simD%>%
  mutate(Clock.Today = ymd_hms(Clock.Today))%>%
   dplyr::filter(Variable=="NodeNumber")%>%
  dplyr::filter(Name=="Iversen_121DefoliationSSFDFD2")%>%
  dplyr::filter(Clock.Today<"2015-01-06")

simD3b<-simD%>%
  mutate(Clock.Today = ymd_hms(Clock.Today))%>%
  dplyr::filter(Variable=="NodeNumber")%>%
  dplyr::filter(Name=="Iversen_121DefoliationSSFDFD2")%>%
  dplyr::filter(Clock.Today>"2015-01-30")
simD3<-rbind(simD3a,simD3b)
  
  
str(simD3)
simD3%>%
 ggplot(aes(x=Clock.Today,y=Predicted))+geom_line(size=1)+theme_bw()+
 facet_wrap(~ID,ncol = 2)+
 geom_point(data=obsFDNTSSF2, aes(x=Clock.Today1, y=Observed),colour="green",size=3)+
 theme(legend.title=element_blank(),legend.position = "blank")+xlab("Date")+ylab("Node number")+
  theme(
     panel.spacing=unit(.01, "lines"),
     panel.border = element_rect(colour = "black",size=1), 
     panel.grid.major = element_blank(),
     panel.grid.minor = element_blank(),
     axis.line = element_line(colour = "black",size = 1),
     axis.text.x = element_text(face = "plain", color = "black", size = 14, vjust=0.5, hjust = 1),
     axis.text.y = element_text(face = "plain", color = "black", size = 14),
     axis.title.x=element_text(face="bold",colour="black",size = 14),
     axis.title.y=element_text(face="bold",colour="black",size = 14),
     strip.background = element_rect(colour = "black", fill = "white",size=1),
     strip.text.x = element_text(size=14, angle=0, face = "plain"), 
     strip.text.y = element_text(size=14, face="plain"),
     legend.title = element_text(colour="black", size=14, face="bold"),
     axis.text = element_text(face = "bold", vjust = 0.5, size = 14))+
  annotate("text", x=ymd_hms("2016-07-24 12:00:00"), y=15, size = 5, label ="paste(R_RMSD == 32.4,'%')", parse=T)
 #ggsave("D:/R/Pictures/C6/NodeNumber/Iversen_121DefoliationSSFDFD2Node.png", width=8, height=4, dpi=500)

```
## F10
```{r,fig.height=4, fig.width=8}

obsFDNTSSF10<-obsFDNT%>%
  dplyr::filter(Variable=="NodeNumber")%>%
  dplyr::filter(Name=="Iversen_121DefoliationSSFDFD10")
  
   
 simD4a<-simD%>%
  mutate(Clock.Today = ymd_hms(Clock.Today))%>%
   dplyr::filter(Variable=="NodeNumber")%>%
  dplyr::filter(Name=="Iversen_121DefoliationSSFDFD10")%>%
  dplyr::filter(Clock.Today<"2015-01-06")

simD4b<-simD%>%
  mutate(Clock.Today = ymd_hms(Clock.Today))%>%
  dplyr::filter(Variable=="NodeNumber")%>%
  dplyr::filter(Name=="Iversen_121DefoliationSSFDFD10")%>%
  dplyr::filter(Clock.Today>"2015-01-30")
simD4<-rbind(simD4a,simD4b)
  
  
str(simD4)
simD4%>%
 ggplot(aes(x=Clock.Today,y=Predicted))+geom_line(size=1)+theme_bw()+
 facet_wrap(~ID,ncol = 2)+
 geom_point(data=obsFDNTSSF10, aes(x=Clock.Today1, y=Observed),colour="green",size=3)+
 theme(legend.title=element_blank(),legend.position = "blank")+xlab("Date")+ylab("Node number")+
  theme(
     panel.spacing=unit(.01, "lines"),
     panel.border = element_rect(colour = "black",size=1), 
     panel.grid.major = element_blank(),
     panel.grid.minor = element_blank(),
     axis.line = element_line(colour = "black",size = 1),
     axis.text.x = element_text(face = "plain", color = "black", size = 14, vjust=0.5, hjust = 1),
     axis.text.y = element_text(face = "plain", color = "black", size = 14),
     axis.title.x=element_text(face="bold",colour="black",size = 14),
     axis.title.y=element_text(face="bold",colour="black",size = 14),
     strip.background = element_rect(colour = "black", fill = "white",size=1),
     strip.text.x = element_text(size=14, angle=0, face = "plain"), 
     strip.text.y = element_text(size=14, face="plain"),
     legend.title = element_text(colour="black", size=14, face="bold"),
     axis.text = element_text(face = "bold", vjust = 0.5, size = 14))+
  annotate("text", x=ymd_hms("2016-07-24 12:00:00"), y=15, size = 5, label ="paste(R_RMSD == 26.3,'%')", parse=T)
 #ggsave("D:/R/Pictures/C6/NodeNumber/Iversen_121DefoliationSSFDFD10Node.png", width=8, height=4, dpi=500)

```
##HH
##F10
```{r,  fig.height=4, fig.width=8}
obsFDNTHHF10<-obsFDNT%>%
  dplyr::filter(Variable=="NodeNumber")%>%
  dplyr::filter(Name=="Iversen_121DefoliationHHFDFD10")

simD5a<-simD%>%
  mutate(Clock.Today = ymd_hms(Clock.Today))%>%
  dplyr::filter(Variable=="NodeNumber")%>%
  dplyr::filter(Name=="Iversen_121DefoliationHHFDFD10")%>%
  dplyr::filter(Clock.Today<"2015-01-06")

simD5b<-simD%>%
  mutate(Clock.Today = ymd_hms(Clock.Today))%>%
  dplyr::filter(Variable=="NodeNumber")%>%
  dplyr::filter(Name=="Iversen_121DefoliationHHFDFD10")%>%
  dplyr::filter(Clock.Today>"2015-01-30")
simD5<-rbind(simD5a,simD5b)
  
  
str(simD5)
simD5%>%
  dplyr::filter(Clock.Today>"2015-01-31")%>%
ggplot(aes(x=Clock.Today,y=Predicted))+geom_line(size=1)+theme_bw()+
  facet_wrap(~ID,ncol = 1)+
  geom_point(data=obsFDNTHHF10, aes(x=Clock.Today1, y=Observed),colour="green",size=3)+
  theme(legend.title=element_blank(),legend.position = "blank")+xlab("Date")+ylab("Node number")+
  #remove grid lines 
   theme(
     panel.spacing=unit(.01, "lines"),
     panel.border = element_rect(colour = "black",size=1), 
     panel.grid.major = element_blank(),
     panel.grid.minor = element_blank(),
     axis.line = element_line(colour = "black",size = 1),
     axis.text.x = element_text(face = "plain", color = "black", size = 14, vjust=0.5, hjust = 1),
     axis.text.y = element_text(face = "plain", color = "black", size = 14),
     axis.title.x=element_text(face="bold",colour="black",size = 14),
     axis.title.y=element_text(face="bold",colour="black",size = 14),
     strip.background = element_rect(colour = "black", fill = "white",size=1),
     strip.text.x = element_text(size=14, angle=0, face = "plain"), 
     strip.text.y = element_text(size=14, face="plain"),
     legend.title = element_text(colour="black", size=14, face="bold"),
     axis.text = element_text(face = "bold", vjust = 0.5, size = 14))+
   annotate("text", x=ymd_hms("2016-07-24 12:00:00"), y=28, size = 5, label ="paste(R_RMSD == 26,'%')", parse=T)
  #ggsave("D:/R/Pictures/C6/NodeNumber/Iversen_121DefoliationHHFDFD10Node.png", width=8, height=4, dpi=500)

  


```
###F2
```{r,  fig.height=4, fig.width=8}
obsFDNTHHF2<-obsFDNT%>%
  dplyr::filter(Variable=="NodeNumber")%>%
  dplyr::filter(Name=="Iversen_121DefoliationHHFDFD2")

simD6a<-simD%>%
  mutate(Clock.Today = ymd_hms(Clock.Today))%>%
  dplyr::filter(Variable=="NodeNumber")%>%
  dplyr::filter(Name=="Iversen_121DefoliationHHFDFD2")%>%
  dplyr::filter(Clock.Today<"2015-01-06")

simD6b<-simD%>%
  mutate(Clock.Today = ymd_hms(Clock.Today))%>%
  dplyr::filter(Variable=="NodeNumber")%>%
  dplyr::filter(Name=="Iversen_121DefoliationHHFDFD2")%>%
  dplyr::filter(Clock.Today>"2015-01-30")
simD6<-rbind(simD6a,simD6b)
  
str(simD6)
simD6%>%
ggplot(aes(x=Clock.Today,y=Predicted))+geom_line(size=1)+theme_bw()+
  facet_wrap(~ID,ncol = 1)+
  geom_point(data=obsFDNTHHF2, aes(x=Clock.Today1, y=Observed),colour="green",size=3)+
  theme(legend.title=element_blank(),legend.position = "blank")+xlab("Date")+ylab("Node number")+
  #remove grid lines 
   theme(
     panel.spacing=unit(.01, "lines"),
     panel.border = element_rect(colour = "black",size=1), 
     panel.grid.major = element_blank(),
     panel.grid.minor = element_blank(),
     axis.line = element_line(colour = "black",size = 1),
     axis.text.x = element_text(face = "plain", color = "black", size = 14, vjust=0.5, hjust = 1),
     axis.text.y = element_text(face = "plain", color = "black", size = 14),
     axis.title.x=element_text(face="bold",colour="black",size = 14),
     axis.title.y=element_text(face="bold",colour="black",size = 14),
     strip.background = element_rect(colour = "black", fill = "white",size=1),
     strip.text.x = element_text(size=14, angle=0, face = "plain"), 
     strip.text.y = element_text(size=14, face="plain"),
     legend.title = element_text(colour="black", size=14, face="bold"),
     axis.text = element_text(face = "bold", vjust = 0.5, size = 14))+
   annotate("text", x=ymd_hms("2016-07-24 12:00:00"), y=27, size = 5, label ="paste(R_RMSD == 27.3,'%')", parse=T)
   #ggsave("D:/R/Pictures/C6/NodeNumber/Iversen_121DefoliationHHFDFD2Node.png", width=8, height=4, dpi=500)



```
# Statistic and Graph
```{r,fig.height=6, fig.width=8}
mergedf
summary(mergedf)
str(mergedf)

mergedf %>%
  dplyr::filter(ExperimentID!="E4")%>%
    dplyr::filter(Variable== "NodeNumber") %>% 
  ggplot(aes(x=Observed, y= Predicted, colour= factor(Name))) +
  geom_point(size=2)+theme_bw()+
  geom_smooth(method = "lm", se = TRUE, linetype = 1, colour="darkgrey") +
  geom_abline(intercept = 0, slope = 1) +
  coord_fixed(ratio = 1)+
  ggtitle("Node Number")+
  facet_wrap(~ID, ncol = 3)+
  theme(legend.title=element_blank(),legend.position = "blank")+xlab("Observed")+ylab("Predicted")+
  theme(
     panel.spacing=unit(.01, "lines"),
     panel.border = element_rect(colour = "black",size=1), 
     panel.grid.major = element_blank(),
     panel.grid.minor = element_blank(),
     axis.line = element_line(colour = "black",size = 1),
     axis.text.x = element_text(face = "plain", color = "black", size = 14, vjust=0.5, hjust = 1),
     axis.text.y = element_text(face = "plain", color = "black", size = 14),
     axis.title.x=element_text(face="bold",colour="black",size = 14),
     axis.title.y=element_text(face="bold",colour="black",size = 14),
     strip.background = element_rect(colour = "black", fill = "white",size=1),
     strip.text.x = element_text(size=14, angle=0, face = "plain"), 
     strip.text.y = element_text(size=14, face="plain"),
     legend.title = element_text(colour="black", size=14, face="bold"),
     axis.text = element_text(face = "bold", vjust = 0.5, size = 14))


   #ggsave("D:/R/Pictures/C6/NodeNumber/Node.png", width=8, height=6, dpi=500)
```
#Statistic and Graph
```{r,fig.height=6, fig.width=8}
mergedf
summary(mergedf)
str(mergedf)
mergedf$FD.x <- factor(mergedf$FD.x, levels=c("FD2","FD5","FD10"))

Figure2<-mergedf %>%
    dplyr::filter(Variable== "NodeNumber") %>% 
    dplyr::filter(Defoliation.x=="HH")%>%
  ggplot(aes(x=Observed, y= Predicted, color=FD.x)) +
  geom_point(size=2)+theme_bw()+
  geom_smooth(method = "lm", se = TRUE, linetype = 1, colour="darkgrey") +
  geom_abline(intercept = 0, slope = 1) +
  coord_fixed(ratio = 1)+
  ggtitle("Node Number")+
  facet_wrap(~FD.x, ncol = 3)+
  scale_x_continuous(breaks = seq(0, 25, by =10), limits=c(0,25))+
  scale_y_continuous(breaks = seq(0, 25, by =10), limits=c(0,25))+
  theme(legend.title=element_blank(),legend.position = "blank")+xlab("Observed")+ylab("Predicted")+
  theme(
     panel.spacing=unit(.01, "lines"),
     panel.border = element_rect(colour = "black",size=1), 
     panel.grid.major = element_blank(),
     panel.grid.minor = element_blank(),
     axis.line = element_line(colour = "black",size = 1),
     axis.text.x = element_text(face = "plain", color = "black", size = 14, vjust=0.5, hjust = 1),
     axis.text.y = element_text(face = "plain", color = "black", size = 14),
     axis.title.x=element_text(face="bold",colour="black",size = 14),
     axis.title.y=element_text(face="bold",colour="black",size = 14),
     strip.background = element_rect(colour = "black", fill = "white",size=1),
     strip.text.x = element_text(size=14, angle=0, face = "plain"), 
     strip.text.y = element_text(size=14, face="plain"),
     legend.title = element_text(colour="black", size=14, face="bold"),
     axis.text = element_text(face = "bold", vjust = 0.5, size = 14))

dat_texta <- data.frame(
  #label =c(a,b,c),
  labela=c("R_RMSE=30.6%","R_RMSE=29.9%","R_RMSE=29.2%"),
  labelb =c("NSE=0.78", "NSE=0.74", "NSE=0.74"),
  FD.x= c("FD2", "FD5", "FD10"),
  x= c(0,  0,  0),
  y1=c(21, 21, 21),
  y2=c(19, 19, 19))

 Figure2+
  geom_text(data=dat_texta, mapping = aes(x=x,y=y1, label = labela),hjust   = -0.1,vjust   = -1,size=4)+
geom_text(data=dat_texta, mapping = aes(x=x,y=y2, label = labelb),hjust   = -0.1,vjust   = -1,size=4)


ggsave("D:/R/Pictures/C6/NodeNumber/NodeHH.png", width=8, height=6, dpi=500)
```


## RMSE

```{r}
str(mergedf)

mergedf %>%
  dplyr::filter(Variable== "NodeNumber") %>% 
  group_by(ID) %>%
  summarise(
    n = n(),
    r2 = gauchStats(Predicted,Observed)[5],
  #  rmse = round(rmse(Predicted,Observed),0),
    r_rmse = round(rmse(Predicted,Observed)/mean(Observed)*100,1),
    nse = round(NSE(Predicted,Observed),2),
    sb = gauchStats(Predicted,Observed)[1],
  nu = gauchStats(Predicted,Observed)[2],
  lc = gauchStats(Predicted,Observed)[3]
  ) 

```

###seedling crop 
###loading observed data
```{r}
upDir <- "D:/R/CombinedData/"
obsData <- "D:/R/CombinedData/"

obsSeedling <- read.table(paste0(obsData, "FallDtreatment.txt"),
                   header = TRUE)
obsSN<- obsSeedling %>%
  dplyr::filter(Variable=="NodeNumber")%>%
  dplyr::filter(ExperimentID=="E4")%>%
  dplyr::filter(Water!="dry")%>%
  mutate(StartDate=dmy(StartDate),MidDate=dmy(MidDate),FinishDate=dmy(FinishDate),Clock.Today=dmy(Clock.Today))%>%
  mutate(GrowthSeason1=as.factor(paste0("Gs_",GrowthSeason))) %>% # creating more intuitive labels here
  mutate(Rotation1=as.factor(paste0("Rt_",Rotation))) %>%
  mutate(ExpUnitCode=as.factor(paste0(Name,GrowthSeason1,Rotation1))) %>%
  mutate(Clock.Today1 = as.POSIXct(paste(Clock.Today,Time),format="%Y-%m-%d %H:%M:%S"))%>%
  mutate(GrowthSeason2=as.factor(paste0("Y",GrowthSeason,"(",Period,")"))) %>%
 mutate(Rotation2=as.factor(paste0("R",Rotation)))
obsSN


```

###seedling
```{r,  fig.height=6, fig.width=8}
obsSNSD<-obsSN%>%
  dplyr::filter(Variable=="NodeNumber")


simD7a<-simD%>%
  mutate(Clock.Today = ymd_hms(Clock.Today))%>%
  dplyr::filter(Clock.Today>"2010-09-27 12:00:00")%>%
  dplyr::filter(Clock.Today<"2012-07-12 12:00:00")%>%
  dplyr::filter(Name==c("Iversen_12SowingDateSD1","Iversen_12SowingDateSD2","Iversen_12SowingDateSD3","Iversen_12SowingDateSD4","Iversen_12SowingDateSD5","Iversen_12SowingDateSD6","Iversen_12SowingDateSD7","Iversen_12SowingDateSD8","Iversen_12SowingDateSD9","Iversen_12SowingDateSD10"))%>%
  dplyr::filter(Variable=="NodeNumber")

SeedND<-data.frame(Name=c("Iversen_12SowingDateSD1","Iversen_12SowingDateSD2","Iversen_12SowingDateSD3","Iversen_12SowingDateSD4","Iversen_12SowingDateSD5","Iversen_12SowingDateSD6","Iversen_12SowingDateSD7","Iversen_12SowingDateSD8","Iversen_12SowingDateSD9","Iversen_12SowingDateSD10"),ID= c("E4DLLS1","E4DLLS2","E4DLLS3","E4DLLS4","E4DLLS5","E4DLLS6","E4DLLS7","E4DLLS8","E4DLLS9","E4DLLS10"))

simD7<-merge(SeedND,simD7a, by=c("Name"))

simD7$ID<-factor(simD7$ID, level=c("E4DLLS1","E4DLLS2","E4DLLS3","E4DLLS4","E4DLLS5","E4DLLS6","E4DLLS7","E4DLLS8","E4DLLS9","E4DLLS10"))

simD7%>%
ggplot(aes(x=Clock.Today,y=Predicted))+geom_line(size=1)+theme_bw()+
  geom_point(data=obsSNSD, aes(x=Clock.Today1, y=Observed),colour="green",size=3)+
   facet_wrap(~ID,ncol = 2)+
  theme(legend.title=element_blank(),legend.position = "blank")+xlab("Date")+ylab("Node number")+
  theme(
     panel.spacing=unit(.01, "lines"),
     panel.border = element_rect(colour = "black",size=1), 
     panel.grid.major = element_blank(),
     panel.grid.minor = element_blank(),
     axis.line = element_line(colour = "black",size = 1),
     axis.text.x = element_text(face = "plain", color = "black", size = 14, vjust=0.5, hjust = 1),
     axis.text.y = element_text(face = "plain", color = "black", size = 14),
     axis.title.x=element_text(face="bold",colour="black",size = 14),
     axis.title.y=element_text(face="bold",colour="black",size = 14),
     strip.background = element_rect(colour = "black", fill = "white",size=1),
     strip.text.x = element_text(size=14, angle=0, face = "plain"), 
     strip.text.y = element_text(size=14, face="plain"),
     legend.title = element_text(colour="black", size=14, face="bold"),
    axis.text = element_text(face = "bold", vjust = 0.5, size = 14))


# dat_text1 <- data.frame(
#   label = c("R_RMSE=27.7%", "R_RMSE=28%", "R_RMSE=21.8%","R_RMSE=24.5%"),
#   ID= c("E2ILLS1","E2ILLS2","E2ILLS3","E2ILLS4"),
#   x= ymd_hms("2001-03-24 12:00:00", "2001-03-24 12:00:00","2001-03-24 12:00:00","2001-03-24 12:00:00"),
#   y=c(17,17,17,17))
# 
#  GSeedling+geom_text(data=dat_text1, mapping = aes(x=x,y=y, label = label),hjust   = -0.1,vjust   = -1,size=5)

#ggsave("D:/R/Pictures/C6/NodeNumber/E4SeedlingNode.png", width=8, height=8, dpi=500)
  
  
```
```{r,fig.height=6, fig.width=8}
simD7b<-simD%>%
  mutate(Clock.Today = ymd_hms(Clock.Today))%>%
  dplyr::filter(Variable=="NodeNumber")%>%
  dplyr::filter(Clock.Today>"2010-09-27 12:00:00")%>%
  dplyr::filter(Clock.Today<"2012-07-12 12:00:00")

obsSNSD1<-obsSNSD%>%
  dplyr::select(Name,ID,Clock.Today,Variable,Observed)

mergedfseed<-merge(obsSNSD1,simD7b,by=c("Name","Variable","Clock.Today"))
mergedfseed$ID<-factor(mergedfseed$ID, level=c("E4DLLS1","E4DLLS2","E4DLLS3","E4DLLS4","E4DLLS5","E4DLLS6","E4DLLS7","E4DLLS8","E4DLLS9","E4DLLS10"))

mergedfseed %>%
  ggplot(aes(x=Observed, y= Predicted, colour= factor(Name))) +
  geom_point(size=2)+theme_bw()+
  geom_smooth(method = "lm", se = TRUE, linetype = 1, colour="darkgrey") +
  geom_abline(intercept = 0, slope = 1) +
  coord_fixed(ratio = 1)+
  ggtitle("Node Number")+
  facet_wrap(~ID, ncol = 4)+
  theme(legend.title=element_blank(),legend.position = "blank")+xlab("Observed")+ylab("Predicted")+
  theme(
     panel.spacing=unit(.01, "lines"),
     panel.border = element_rect(colour = "black",size=1), 
     panel.grid.major = element_blank(),
     panel.grid.minor = element_blank(),
     axis.line = element_line(colour = "black",size = 1),
     axis.text.x = element_text(face = "plain", color = "black", size = 14, vjust=0.5, hjust = 1),
     axis.text.y = element_text(face = "plain", color = "black", size = 14),
     axis.title.x=element_text(face="bold",colour="black",size = 14),
     axis.title.y=element_text(face="bold",colour="black",size = 14),
     strip.background = element_rect(colour = "black", fill = "white",size=1),
     strip.text.x = element_text(size=14, angle=0, face = "plain"), 
     strip.text.y = element_text(size=14, face="plain"),
     legend.title = element_text(colour="black", size=14, face="bold"),
     axis.text = element_text(face = "bold", vjust = 0.5, size = 14))


   #ggsave("D:/R/Pictures/C6/NodeNumber/SeedlingNode.png", width=8, height=6, dpi=500)
```
## RMSE

```{r}
str(mergedfseed)

mergedfseed %>%
  dplyr::filter(Variable== "NodeNumber") %>% 
  group_by(ID) %>%
  summarise(
    n = n(),
    r2 = gauchStats(Predicted,Observed)[5],
  #  rmse = round(rmse(Predicted,Observed),0),
    r_rmse = round(rmse(Predicted,Observed)/mean(Observed)*100,1),
    nse = round(NSE(Predicted,Observed),2),
    sb = gauchStats(Predicted,Observed)[1],
  nu = gauchStats(Predicted,Observed)[2],
  lc = gauchStats(Predicted,Observed)[3]
  ) 

```



































                      
