...
### merge tables for root respiration 
```{r}
upDir <- "D:/R/TtAll/"
obsData <- "D:/R/TtAll/"

RootMet <- read.table(paste0(obsData, "RootMet1.txt"),
                   header = TRUE)
RootMet1<-RootMet%>%
  mutate(ExpUnitCode=ExpName)%>%
  mutate(Clock.Today=dmy(Clock.Today))%>%
  dplyr::select(Clock.Today,ExpUnitCode)

obsA1<-obsA%>%
  dplyr::filter(Variable=="LAI")

mergemet1<-merge(RootMet1,obsA1,by=c("Clock.Today","ExpUnitCode"))
write.csv(mergemet1,"D:/R/TtAll/mergemet1.CSV", row.names = FALSE)
...
mergemet2<-merge(RootMet1,obADF1,by=c("Clock.Today","ExpUnitCode"))

write.csv(mergemet2,"D:/R/TtAll/mergemet2.CSV", row.names = FALSE)
...
RUEshoots<-RUEshootp%>%
  group_by(Name,GrowthSeason,Rotation,GrowthRotation,Tmean,Ppm,Stage,Trend,ID,ExperimentID,StartDate,MidDate,FinishDate) %>%
...
RUE.shoot%>%
  dplyr::filter(RUEs1>0)%>%
  dplyr::filter(GrowthRotation!="17")%>%
  ggplot(aes(y=RUEs1, x=Tmean,color=ID,label=GrowthRotation))+geom_text()+theme_bw()+ylab("Shoot RUE (g/MJ)")+xlab("Mean temperature")+mytheme3+
 #geom_smooth(method = "lm", se = TRUE, linetype = 1, colour="blue")+
 facet_grid(Trend~Stage)+
  theme(axis.title.x=element_text(face="bold",colour="black",size = 12))+
 theme(axis.title.y=element_text(face="bold",colour="black",size = 12))
...
###confidence intrvals of 95%

RUE.lm<-lm(RUEt1~Tmean,data = RUETotala)
summary(RUE.lm)
new.data <-data.frame(Tmean=18)
predict(RUE.lm, newdata = new.data, interval='confidence') 
...
obsYield1<-obsYield%>%
 ...
 mytheme2+
 annotate("text", x=ymd_hms("1999-10-24 12:00:00"), y=6500, size = 5, label ="paste(R_RMSD == 53.7,'%')", parse=T)
...
ggplot(aes(x=Clock.Today,y=Predicted))+geom_line(size=1)+theme_bw()+
  ...
  theme(legend.title=element_blank(),legend.position = "blank")+xlab("Date")+ylab(bquote(bold('Shoot DM ('*kg~ha^-1*')')))+
 mytheme2+
  annotate("text", x=ymd_hms("2003-07-24 12:00:00"), y=6000, size = 5, label ="paste(R_RMSD == 27.1,'%')", parse=T)
 #geom_text(aes(x=ymd_hms("2003-07-24 12:00:00"), y=750, label="R_RMSD = 27.1%",size=5))
 ggsave("D:/R/Pictures/Shoot/Iversen_91DefoliationLLshoot1.png", width=8, height=4, dpi=500)
...
dat_text3 <- data.frame(
  label = c("R_RMSE=41.1%","R_RMSE=39.4%","R_RMSE=53.2%", "R_RMSE=59.2%"),
  ...
  x= ymd_hms("2001-01-24 12:00:00", "2001-01-24 12:00:00","2001-01-24 12:00:00","2001-01-24 12:00:00"),
   y=c(5500,5500,5500,5500)) 

 f<-p1a  +geom_text(data=dat_text3, mapping = aes(x=x,y=y, label = label),hjust   = -0.1,vjust   = -1,size=5)+mytheme3
 f
...
ggplot(aes(x=Clock.Today,y=Predicted))+geom_line(size=1)+theme_bw()+
  ...
  theme(legend.title=element_blank(),legend.position = "blank")+xlab("Date")+ylab(bquote(bold('Shoot DM ('*kg~ha^-1*')')))+
  mytheme2+
  annotate("text", x=ymd_hms("2016-07-24 12:00:00"), y=8500, size = 5, label ="paste(R_RMSD == 92.3,'%')" , parse=T)
...
mergedf %>%
  ...
  #dplyr::filter(Defoliation.x== "LL")%>%
  #dplyr::filter(Collection=="2014_2018")%>%
  #dplyr::filter(Clock.Today<"2015-05-01")%>%
  ggplot(aes(x=Observed, y= Predicted, colour= factor(Name))) +
  geom_point(size=2)+theme_bw()+
  geom_smooth(method = "lm", se = TRUE, linetype = 1, colour="darkgrey") +
  geom_abline(intercept = 0, slope = 1) +
...
head(ObsR)
mergedfroot<-merge(ObsR,simD,by=c("Clock.Today","Name","Variable"))
summary(mergedfroot)
str(mergedfroot)
mergedfroot
...
ggplot(aes(x=Clock.Today,y=Predicted))+geom_line(size=1)+theme_bw()+
  ...
  #ggtitle(paste0("E3ILL","(Iversen_91DefoliationLL)"))+
  ...
  #remove grid lines 
   ...
   ggsave("D:/R/Pictures/Shoot/rootshoot1.png", width=8, height=5, dpi=500)
...
ggplot(aes(x=Clock.Today,y=Predicted))+geom_line(size=1)+theme_bw()+
  facet_grid(Variable~ID)+
  #ggtitle(paste0("E5ILLFD5","(Iversen_121DefoliationLLFDFD5)"))+
 ...
 #remove grid lines 
   theme(
     ...
     axis.text = element_text(face = "bold", vjust = 0.5, size = 14))
   #annotate("text", x=ymd_hms("2016-07-24 12:00:00"), y=18, size = 5, label ="paste(R_RMSD == 64.5,'%')", parse=T)
   ggsave("D:/R/Pictures/Shoot/rootshoot2.png", width=8, height=5, dpi=500)
