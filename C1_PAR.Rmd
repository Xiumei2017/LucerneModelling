...
ObsPL1%>%
 ...
 annotate("text", x=1.5, y=1, size = 5, label ="paste(y == 1-exp(-0.76*x),~~R^2==0.93 )", parse=T)
  #ggsave("D:/R/Pictures/PAR/Regrowth.png", width=8, height=5  , dpi=500) 
```
###seedling crop
```{r,fig.height=5, fig.width=10}
test1 <- function(x) {1-exp(-0.61*x)}
stat_function(fun = test1)
...
ObsPL1%>%
 ...
 annotate("text", x=1.5, y=1, size = 5, label ="paste(y == 1-exp(-0.61*x),~~R^2==0.90 )", parse=T)
...
Exc.K%>%
  ggplot(aes(x=Observed.x,y=lnFI,color=Name,label=GrowthRotation,shape=Stage))+geom_text()+theme_bw()+xlab("ln(I/IO)")+ylab(bquote(bold('LAI ('*m^2~m^-2*')')))+
  #geom_point(size=2)+
