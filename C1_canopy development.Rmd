...
obsAll <- read.table(paste0(obsData, "ObsAllLAI.txt"),
                   header = TRUE)
obsA<- obsAll %>%
...
```
###LAI PER Node per Tt aganist Pp (test a crazy idea!!!)
```{r}
Leaf.node<-Leaf_Node%>%
  dplyr::filter(NodeNumber.y<=11)%>%
  mutate(Leaf.node=LAI/NodeNumber.y)

Leaf.node.Tt<-Leaf.node%>%
  group_by(Name,Collection,GrowthRotation,Stage,Trend,ExpUnitCode,Ppm,Tmean)%>%
  do(mod = lm(Leaf.node~Tt_broken_sum,data=.)) %>%
  mutate(slopeLNT = summary(mod)$coeff[2]) %>%
  dplyr::select(-mod)%>%
  dplyr::filter(slopeLNT>0)

Leaf.node.Tt%>%
  ggplot(aes(y=slopeLNT, x=Ppm,color=Name,label=GrowthRotation))+geom_text()+theme_bw()+xlab("Photoperiod (h)")+ylab("LAI/Node/thermal time")+ 
  geom_smooth(method = "lm", se = TRUE,linetype=1 , colour="black")+
 facet_grid(Trend~Stage)+ggtitle("LAI/Node/°Cd")+
  theme(axis.title.x=element_text(face="bold",colour="black",size = 12))+
 theme(axis.title.y=element_text(face="bold",colour="black",size = 12))
...
obsLAI%>%
  ggplot(aes(x=Tt_broken_sum, y=Observed))+geom_point(size=2)+theme_bw()+xlab("Thermal time(°Cd)")+ylab("LAI (m^2/m^2)")+ 
...
obsNode1<-ObsL%>%
  ...
  dplyr::filter(Variable=="NodeNumber")
obsLAI1%>%
  ggplot(aes(x=Tt_broken_sum, y=Observed))+geom_point(size=2)+theme_bw()+xlab("Thermal time(°Cd)")+ylab("Node Number(m^2/m^2) ")+ggtitle("Iversen_91DefoliationLL")+
