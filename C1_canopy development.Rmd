...
TtA
ObsL <-merge(obsA,TtA,by=c("Clock.Today","ExpUnitCode")) %>%
...
obsLAI1%>%
  ggplot(aes(x=Tt_broken_sum, y=Observed), colour=factor(Name))+
  geom_point(size=2)+theme_bw()+xlab("Thermal time(°Cd)")+ylab("LAI (m^2/m^2) ")+ggtitle("Iversen_91DefoliationLL")+
...
obsLAI5%>%
  ggplot(aes(x=Tt_broken_sum, y=Observed), colour=factor(Name))+geom_point(size=2)+theme_bw()+xlab("Thermal time(°Cd)")+ylab("LAI (m^2/m^2) ")+ggtitle("Iversen_9SowingDateSD4Waterirr")+ facet_grid(GrowthSeason.x~Rotation.x)+
  #geom_smooth(method = "glm",family = gaussian(link="log"), aes(colour = "Exponential"))+  
